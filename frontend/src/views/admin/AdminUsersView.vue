<template>
  <AdminLayout>
    <div>
      <h1 class="text-3xl font-bold text-dark mb-8">Gestión de Usuarios</h1>
          ...
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Registro</th>
              ...
              >
                <td class="px-6 py-4">
                  <div class="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-gray-600">{{ user.phone || 'Sin teléfono' }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <select
                    ...
                    class="px-3 py-1 text-xs rounded-full font-semibold border cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary"
                ...
                <td class="px-6 py-4">
                  <p class="text-sm text-gray-700">{{ formatDate(user.created_at) }}</p>
                </td>
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t">
              <div>
                ...
                <p class="font-semibold capitalize">{{ selectedUser.role }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-600">Estado</p>
                <p class="font-semibold">{{ selectedUser.is_active ? 'Activo' : 'Inactivo' }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-600">Teléfono</p>
                <p class="font-semibold">{{ selectedUser.phone || 'No especificado' }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-600">Documento</p>
                <p class="font-semibold">{{ selectedUser.document || 'No especificado' }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-600">Fecha de registro</p>
                <p class="font-semibold">{{ formatDate(selectedUser.created_at) }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-600">Última actualización</p>
                <p class="font-semibold">{{ formatDate(selectedUser.updated_at) }}</p>
              </div>
            </div>
          </div>
        </div>
